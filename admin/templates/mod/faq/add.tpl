...
<table width="98%" border="0" align="center" cellpadding="4" cellspacing="1" class="content" bgcolor="#ffffff">
  <tr>
    <td colspan="2" height="25" class="title">选择要编辑的栏目</td>
  </tr>
  <tr>
    <td width="4%" class="line">栏目</td>
    <td width="96%" class="line">
    	<select id="cate">
        	{foreach from=$cate item=list}
            	<optgroup label="{$list.name}">
                	{foreach from=$list.item item=val}
                    	<option value="{$val.id}" {if $val.id == $item.id}selected="selected"{/if}>{$val.name}</option>
                    {/foreach}
                </optgroup>
            {/foreach}
    	</select>
  ...
  <tr>
    <td>&nbsp;</td>
    <td><input type="button" name="button" id="button" value="打开栏目" class="btn" onclick="location = '?id=' + $('#cate option:selected').val()" /></td>
  </tr>
...
<table width="98%" border="0" align="center" cellpadding="4" cellspacing="1" class="content" bgcolor="#ffffff">
  <tr>
    <td colspan="2" height="25" class="title">编辑 {$item.name}</td>
  </tr>
  <tr>
    <td width="4%" class="line">内容<input name="Type" value="{$item.id}" type="hidden" /></td>

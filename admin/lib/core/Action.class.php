...
class Action 
{
	...
	{
		$this->app = $app;
		global $admin_group_auth;
		if (!defined("NO_LOGIN") || NO_LOGIN != true)
		{
			if (empty($_SESSION['admin_uid']))
			{
				$this->showMsg('请先登录', 'index.php', '_top');
			}
			if (isset($admin_group_auth) && $_SESSION['admin_info']['gid'] != 0)
			{
				if (!in_array($admin_group_auth, $_SESSION['admin_info']['group']))
				{
					$this->showMsg("您已经登录, 但无权限访问此页. 请更换用户后再试!", 'admin.php', '_top');
				}
			}
		}
	}
